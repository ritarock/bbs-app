...
type Comment struct {
	...
	// CommentedAt holds the value of the "commented_at" field.
	...
	for i := range columns {
		...
		case comment.FieldCommentedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field commented_at", values[i])
			} else if value.Valid {
				c.CommentedAt = value.Time
			}
...
func (c *Comment) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("commented_at=")
	builder.WriteString(c.CommentedAt.Format(time.ANSIC))
	builder.WriteString(", ")
